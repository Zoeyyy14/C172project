AFRAME.registerComponent("create-markers", {
    init:async function(){
      ...
      dishes.map(dish=>{
        ...
        toyTitle.setAttribute("id",`toy-title-${dish.id}`)
        toyTitle.setAttribute("position",{x:0,y:0,z:0.1})
        toyTitle.setAttribute("rotation",{x:0,y:0,z:0})
        toyTitle.setAttribute("text",{font:"monoid",color:"black",width:1.8,height:1,align:center,value:dish.dish_name.toUpperCase()})           
        titlePlane.appendChild(typeTitle)

      var pricePlane = document.createElement("a-image");
      pricePlane.setAttribute("id", `price-plane-${toy.id}`);
      pricePlane.setAttribute("position", { x: -1.5, y: 0, z: 0.3 });
